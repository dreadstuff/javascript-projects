...
while (startFuelLevel < 5000 || startFuelLevel > 30000 || isNaN(startFuelLevel)) {
  startFuelLevel = input.question("Provide the starting fuel level between 5000 and 30000: ");
}

console.log("Starting fuel level:", startFuelLevel);

//b. Use a second loop to query the user for the number of astronauts (up to a maximum of 7). Validate the entry.
while (astronautsAboard <= 0 || astronautsAboard > 7 || isNaN(astronautsAboard)) {
  astronautsAboard = input.question("How many astronauts are onboard?: ");
}
  
console.log(`There are ${astronautsAboard} total astronauts onboard.`);
